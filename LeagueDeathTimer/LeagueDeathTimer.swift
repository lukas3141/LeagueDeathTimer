...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    static var gameClientBundleID =
        "com.riotgames.LeagueofLegends.GameClient"
    static var leagueClientUxBundleID =
        "com.riotgames.LeagueofLegends.LeagueClientUx"

    func applicationDidFinishLaunching(_ notification: Notification) {
        NSApp.setActivationPolicy(.accessory)
        leagueAppPath =
            UserDefaults.standard.value(forKey: "leagueAppPath")
            as? String

        let leagueCustomPathPlaceholderText =
            "Override this with the path to your League of Legends app"
        let defaultLeagueAppPath = "/Applications/League of Legends.app"

        if leagueAppPath == nil {
            UserDefaults.standard.setValue(
                leagueCustomPathPlaceholderText,
                forKey: "leagueAppPath")
            leagueAppPath = defaultLeagueAppPath
        } else if let leagueAppPathStr = leagueAppPath,
            leagueAppPathStr == leagueCustomPathPlaceholderText
        {
            leagueAppPath = defaultLeagueAppPath
        }
        ...
        if let button = statusItem?.button {
            ...
            let aspectRatio =
                button.image!.size.height
                / button.image!.size.width
        ...
        let quitItem = NSMenuItem(
            title: "Quit", action: #selector(quitApp),
            keyEquivalent: "")
    ...
    func waitForLCUApi() {
        ...
        Task {
            while true {
                if let lcuCreds = fetchLCUCredentials(
                    mainAppPackagePath: URL(
                        string: leagueAppPath!)!),
                    let locale = await getClientLocale(
                        lcuCreds: lcuCreds)
                {
    ...
    @objc private func toggleLaunchAtLogin() {
        if let launchAtLoginItem = statusItem?.menu?.item(withTag: 1) {
            ...
            launchAtLoginItem.state =
                LaunchAtLogin.isEnabled ? .on : .off
        }
    }
    ...
    private func openMainWindow() {
        ...
        deathtimerwindow = NSWindow(
            contentRect: NSRect(
                x: 0, y: 0, width: 517, height: 138),
        ...
        deathtimerwindow?.contentView = NSHostingView(
            rootView: contentView)
        deathtimerwindow?.level = NSWindow.Level.screenSaver
        if let winOriginX = UserDefaults.standard.value(
            forKey: "winOriginX")
            as? CGFloat,
            let winOriginY = UserDefaults.standard.value(
                forKey: "winOriginY")
                as? CGFloat
        {
    ...
    private func closeMainWindow() {
        if let deathtimerwindow = deathtimerwindow {
            ...
            UserDefaults.standard.set(
                winOrigin.x, forKey: "winOriginX")
            UserDefaults.standard.set(
                winOrigin.y, forKey: "winOriginY")
        }
